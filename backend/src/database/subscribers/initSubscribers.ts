/*
This file is part of the SoLawi Bedarf app

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program.  If not, see <https://www.gnu.org/licenses/>.
*/

import { ChangeBatcher } from "./ChangeBatcher";
import { changeBatcherRegistry } from "./ChangeBatcherRegistry";

/**
 * Order change batcher instance.
 * This batches order change notifications to avoid triggering calculations
 * multiple times when many changes happen in a single transaction.
 *
 * This batcher is automatically registered in the global registry and will be
 * flushed by the flushAllChangeBatchers middleware.
 */
export const orderChangeBatcher = new ChangeBatcher<number>("Order", 50);

// Explicitly register to ensure it's available immediately (before async import completes)
changeBatcherRegistry.register(orderChangeBatcher);

/**
 * Initialize subscribers and set their calculation callbacks.
 * This is where you configure what happens when entities change.
 *
 * Example for adding a new entity subscriber:
 * ```typescript
 * import { ChangeBatcher } from "./ChangeBatcher";
 * import { changeBatcherRegistry } from "./ChangeBatcherRegistry";
 * import { createEntityChangeSubscriber } from "./EntityChangeSubscriber";
 * import { Product } from "../Product";
 *
 * // Create batcher
 * export const productBatcher = new ChangeBatcher<number>("Product", 50);
 *
 * // Register for automatic flushing (required!)
 * changeBatcherRegistry.register(productBatcher);
 *
 * // Create subscriber
 * export const productSubscriber = createEntityChangeSubscriber(
 *   Product,
 *   (product) => product.id,
 *   productBatcher
 * );
 *
 * // Set calculation callback
 * productBatcher.setCalculationCallback(async (affectedProductIds) => {
 *   await yourCalculationService.recalculateProducts(affectedProductIds);
 * });
 * ```
 */
orderChangeBatcher.setCalculationCallback(async (affectedOrderIds) => {
  // TODO: Implement your heavy calculation here
  // Example:
  // await yourCalculationService.recalculateAllOrders();
  console.log("Order changes detected. Affected order IDs:", affectedOrderIds);
});
